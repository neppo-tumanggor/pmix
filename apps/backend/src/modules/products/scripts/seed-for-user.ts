...
async function main() {
  console.log('\n🌱 Seed Products for Logged-in User');
  ...
  try {
    // Get all users to see available tenants
    const users = await dataSource.query('SELECT id, email, tenant_id FROM users WHERE tenant_id IS NOT NULL');
    console.log('\n👥 Available users with tenants:');
    ...
    rl.question('\n❓ Enter the tenant ID you want to seed products for: ', async (tenantId: string) => {
      rl.close();

      if (!tenantId.trim()) {
        console.log('❌ Tenant ID cannot be empty');
        process.exit(1);
      }
      ...
      console.log('\n🗑️  Clearing existing data for this tenant...');
